...
```{r}
# 1) We will practice identifying whether the treatment and control are different for Color. First, take a look at the structure of the dataframe with the "str" function. Insert the word `WePhones` in between the () in the structure function below:
...
# 3) Now, we will calculate the percent of phones in the treatment group and control group that are "Black". We provide an example of how to do this for the Treatment group. Use this example for our `Control` dataframe:
...
# 2) So are there any variables that show a statistically significant amount of imbalance? To find out, we will first remind ourselves what variables are in these dataframes with the str() command:

    str(Treatment)
    
# 3) Now use the `t.test` function to search through each variable in the `Treatment and `Control` groups until you find one that is imbalanced (i.e. where the probability that the mean difference between the specification in the treatment and control groups is less than 0.05). 
...
# 4) Now write the name of the unbalanced trait between control and treatment groups into Solution1.
